...
@Component({
  selector: 'line-chart-daily',
  standalone: true,
  template: ` <div #chartContainer class="w-full h-[200px]"></div> `,
})
export class LineChartDailyComponent implements OnInit, AfterViewInit {
  @Input() app!: string;
  ...
  ngOnInit() {
    this.metrics.getDailyP95(this.app, this.scenario).subscribe((points) => {
      this.points = points;
      this.updateChart();
    });
  }
