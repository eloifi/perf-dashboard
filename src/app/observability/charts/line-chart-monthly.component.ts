...
@Component({
  selector: 'line-chart-monthly',
  standalone: true,
  template: ` <div #chartContainer class="w-full h-[320px]"></div> `,
})
export class LineChartMonthlyComponent implements OnInit, AfterViewInit {
  @Input() app!: string;
  ...
  ngOnInit() {
    this.metrics.getMonthlyP95(this.app, this.scenario).subscribe((points) => {
      this.points = points;
      this.updateChart();
    });
  }
